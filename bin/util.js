...
export const createAsyncPool = (poolCap, keepOrder = true) => async (iterable, fun) => {
  const pool = new Set();
  ...
  for (const index in iterable) {
    ...
    const promise = (async () => fun(item))().then(value =>
      [promise, value, index]
    )
...
export const scoreTestResult = (result, testsDir = "") => {
...
export const scoreTestResults = (results, testsDir = "") => {
  const passedCount = results.reduce((passedCount, result) =>
    scoreTestResult(result, testsDir) ? passedCount + 1 : passedCount,
    0
  );

  console.log();
  console.log(cli.bold(`Overall: ${passedCount}/${results.length} - suite ${passedCount === results.length ? cli.green("passed") : cli.red("failed")}`));
  console.log();
}

export const runTestCase = filepath => new Promise(resolve => {
  const testProcess = spawn("antlr4-parse", ["ShortScript.g4", "program", filepath]);
  let passed = true;
  let error = "";
