...
const testCasePaths = (
	await readdir(join(__dirname, "..", testsDir), {
		withFileTypes: true,
		recursive: true,
		encoding,
	})
...
const taskPoolRunner = createAsyncPool<string>(poolCap);
const results = await taskPoolRunner(testCasePaths, async (testCasePath) => ({
	testCasePath,
	...(await runTestCase(testCasePath)),
}));
